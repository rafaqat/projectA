...
export class Indexer {
  ...
  private async deriveFile(
    ...
    await inScope(scope, async (trx) => {
      // A file's symbols, citation blocks and chunks are the ingest's hot loop; a round-trip per row
      // was the throughput ceiling. Build the rows, then insert each table in multi-row statements
      // (500/statement, like symbol_references). UUIDs are client-generated with no RETURNING, so a
      // batch is subject to the same per-row RLS INSERT policy the single inserts were.
      const symbolIds = parsed!.symbols.map(() => randomUUID())
      const symbolRows = parsed!.symbols.map((symbol, si) => ({
        id: symbolIds[si],
        workspace_id: workspaceId,
        commit_id: commitId,
        path: file.path,
      ...
      const blockRows = parsed!.symbols.flatMap((symbol, si) =>
        symbol.blocks.map((block, ordinal) => ({
          id: randomUUID(),
          workspace_id: workspaceId,
          symbol_id: symbolIds[si],
          ordinal,
          start_line: block.startLine,
          end_line: block.endLine,
        }))
      )
      for (let b = 0; b < symbolRows.length; b += 500)
        await trx.table('symbols').insert(symbolRows.slice(b, b + 500))
      for (let b = 0; b < blockRows.length; b += 500)
        await trx.table('citation_blocks').insert(blockRows.slice(b, b + 500))
      outcome.symbols += symbolRows.length
      ...
      for (let b = 0; b < chunkRows.length; b += 500)
        await trx.table('chunks').insert(chunkRows.slice(b, b + 500))
      outcome.chunks += chunkRows.length
    })
  }
